 ...
 */
public abstract class Tuple3f<T extends Tuple3f<?>> implements Serializable, Cloneable, Comparable<T> {
	...
	/**
	 * Sets the value of this tuple to the difference of itself and x and y and z.
	 * @param x
	 * @param y
	 * @param z
	 */
	...
	public boolean epsilonNul(float epsilon) {
		...
		if((this.z<0f?-this.z:this.z) > epsilon) return false;
		return true;
	}
	...
	public int compareTo(T o) {
		...
		c = Float.compare(this.y, o.y);
		if (c != 0) {
			return c;
		}
		c = Float.compare(this.z, o.z);
		return Float.compare(this.z, o.z);
	}
	
	/** Replies if the tuple has zero coordinates.
	 *
	 * @return <code>true</code> if the x and y and z coordinates are equal to zero.
	 */
